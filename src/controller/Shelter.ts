...
import { createShelterService, updateShelterService, deleteShelterService} from "../service/ShelterService";

export async function createShelter(req: Request, res: Response) {
    const { name, address, long, lat, image, city, state, zip, phone, code_pix, website, description, item, capacity, current } = req.body;
    const id = req.user.id;
    const shelterModel = new ShelterModel(
        name, address, city, image, state, zip, phone, code_pix, website, description, long, lat, id, item, capacity, current
    );
...
export async function updateShelter(req: Request, res: Response) {
    ...
    const id = req.user.id
    const user_id = id;
    const shelterModel = new ShelterModel(name, address, long, lat, image, city, state, zip, phone, code_pix, website, description, user_id, item, capacity, current)
